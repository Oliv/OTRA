...
abstract class PHPWebSocket
{
	...
	protected $wsTimers     = array();

    // communication functions
    abstract protected function wsOnMessage($clientID, $message, $messageLength, $binary);
    abstract protected function wsOnOpen($clientID);
    abstract protected function wsOnClose($clientID, $status);
	...
	*/
	...
	public function wsStartServer($host, $port) {
		...
		while (isset($this->wsRead[0])) {
            ...
            foreach ($this->wsTimers as $k => $timer) {
                if (time() >= $timer['current']) {
                    if (method_exists($this, $timer['function'])) {
                        call_user_func(array($this, $timer['function']), $timer['id']);
                    }
                    $this->wsTimers[$k]['current'] = time() + rand($timer['inc']['min'], $timer['inc']['max']);
                }
            }
		}
	...
	protected function wsCheckIdleClients() {
		$time = time();
		foreach ($this->wsClients as $clientID => $client) {
			if ($client['ReadyState'] != self::WS_READY_STATE_CLOSED) {
				...
				elseif ($time >= $client['LastRecvTime'] + self::WS_TIMEOUT_RECV) {
					...
					if ($client['ReadyState'] != self::WS_READY_STATE_CONNECTING) {
						...
						$this->wsClients[$clientID]['PingSentTime'] = time();
	...
	private function wsAddClient($socket, $clientIP) {
		...
		$this->wsClients[$clientID] = array(
            'Socket'                    => $socket,
            'MessageBuffer'             => '',
            'ReadyState'                => self::WS_READY_STATE_CONNECTING,
            'LastRecvTime'              => time(),
            'PingSentTime'              => false,
            'CloseStatus'               => 0,
            'IPv4'                      => $clientIP,
            'FramePayloadDataLength'    => false,
            'FrameBytesRead'            => 0,
            'FrameBuffer'               => '',
            'MessageOpcode'             => 0,
    ...
    private function wsRemoveClient($clientID) {
		// fetch close status (which could be false), and call wsOnClose
		$this->wsOnClose($clientID, $this->wsClients[$clientID]['CloseStatus']);

		// close socket
		$socket = $this->wsClients[$clientID]['Socket'];
		socket_close($socket);

		// decrease amount of clients connected on this client's IP
		$clientIP = $this->wsClients[$clientID]['IPv4'];
	...
	private function wsGetClientSocket($clientID) {
		return $this->wsClients[$clientID]['Socket'];
	}

	// client read functions
	private function wsProcessClient($clientID, &$buffer, $bufferLength) {
		if ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_OPEN) {
			// handshake completed
			$result = $this->wsBuildClientFrame($clientID, $buffer, $bufferLength);
		}
		elseif ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CONNECTING) {
			// handshake not completed
			$result = $this->wsProcessClientHandshake($clientID, $buffer);
			if ($result) {
				$this->wsClients[$clientID]['ReadyState'] = self::WS_READY_STATE_OPEN;

				$this->wsOnOpen($clientID);
			}
		}
		else {
    ...
    private function wsBuildClientFrame($clientID, &$buffer, $bufferLength) {
		// increase number of bytes read for the frame, and join buffer onto end of the frame buffer
		$this->wsClients[$clientID]['FrameBytesRead'] += $bufferLength;
		$this->wsClients[$clientID]['FrameBuffer'] .= $buffer;

		// check if the length of the frame's payload data has been fetched, if not then attempt to fetch it from the frame buffer
		if ($this->wsClients[$clientID]['FramePayloadDataLength'] !== false || $this->wsCheckSizeClientFrame($clientID) == true) {
			// work out the header length of the frame
			$headerLength = ($this->wsClients[$clientID]['FramePayloadDataLength'] <= 125 ? 0 : ($this->wsClients[$clientID]['FramePayloadDataLength'] <= 65535 ? 2 : 8)) + 6;

			// check if all bytes have been received for the frame
			$frameLength = $this->wsClients[$clientID]['FramePayloadDataLength'] + $headerLength;
			if ($this->wsClients[$clientID]['FrameBytesRead'] >= $frameLength) {
				// check if too many bytes have been read for the frame (they are part of the next frame)
				$nextFrameBytesLength = $this->wsClients[$clientID]['FrameBytesRead'] - $frameLength;
				if ($nextFrameBytesLength > 0) {
					$this->wsClients[$clientID]['FrameBytesRead'] -= $nextFrameBytesLength;
					$nextFrameBytes = substr($this->wsClients[$clientID]['FrameBuffer'], $frameLength);
					$this->wsClients[$clientID]['FrameBuffer'] = substr($this->wsClients[$clientID]['FrameBuffer'], 0, $frameLength);
				}
				...
				if (isset($this->wsClients[$clientID])) {
					$this->wsClients[$clientID]['FramePayloadDataLength'] = false;
					$this->wsClients[$clientID]['FrameBytesRead'] = 0;
					$this->wsClients[$clientID]['FrameBuffer'] = '';
				}
    ...
    private function wsCheckSizeClientFrame($clientID) {
		// check if at least 2 bytes have been stored in the frame buffer
		if ($this->wsClients[$clientID]['FrameBytesRead'] > 1) {
			// fetch payload length in byte 2, max will be 127
			$payloadLength = ord(substr($this->wsClients[$clientID]['FrameBuffer'], 1, 1)) & 127;

			if ($payloadLength <= 125) {
				// actual payload length is <= 125
				$this->wsClients[$clientID]['FramePayloadDataLength'] = $payloadLength;
			}
			elseif ($payloadLength == 126) {
				// actual payload length is <= 65,535
				if (substr($this->wsClients[$clientID]['FrameBuffer'], 3, 1) !== false) {
					// at least another 2 bytes are set
					$payloadLengthExtended = substr($this->wsClients[$clientID]['FrameBuffer'], 2, 2);
					$array = unpack('na', $payloadLengthExtended);
					$this->wsClients[$clientID]['FramePayloadDataLength'] = $array['a'];
				}
			}
			else {
				// actual payload length is > 65,535
				if (substr($this->wsClients[$clientID]['FrameBuffer'], 9, 1) !== false) {
					// at least another 8 bytes are set
					$payloadLengthExtended = substr($this->wsClients[$clientID]['FrameBuffer'], 2, 8);
			...
			if ($this->wsClients[$clientID]['FramePayloadDataLength'] !== false) {

				// check if the frame's payload data length exceeds self::WS_MAX_FRAME_PAYLOAD_RECV
				if ($this->wsClients[$clientID]['FramePayloadDataLength'] > self::WS_MAX_FRAME_PAYLOAD_RECV) {
					$this->wsClients[$clientID]['FramePayloadDataLength'] = false;
					$this->wsSendClientClose($clientID, self::WS_STATUS_MESSAGE_TOO_BIG);
					return false;
				}
				...
				$controlFrame = (ord(substr($this->wsClients[$clientID]['FrameBuffer'], 0, 1)) & 8) == 8;
				if (!$controlFrame) {
					$newMessagePayloadLength = $this->wsClients[$clientID]['MessageBufferLength'] + $this->wsClients[$clientID]['FramePayloadDataLength'];
    ...
    private function wsProcessClientFrame($clientID) {
		// store the time that data was last received from the client
		$this->wsClients[$clientID]['LastRecvTime'] = time();

		// fetch frame buffer
		$buffer = &$this->wsClients[$clientID]['FrameBuffer'];
		...
		$seek = $this->wsClients[$clientID]['FramePayloadDataLength'] <= 125 ? 2 : ($this->wsClients[$clientID]['FramePayloadDataLength'] <= 65535 ? 4 : 10);
		...
		if ($opcode != self::WS_OPCODE_CONTINUATION && $this->wsClients[$clientID]['MessageBufferLength'] > 0) {
			// clear the message buffer
			$this->wsClients[$clientID]['MessageBufferLength'] = 0;
			$this->wsClients[$clientID]['MessageBuffer'] = '';
		}
		...
		if ($fin == self::WS_FIN) {
			...
			if ($opcode != self::WS_OPCODE_CONTINUATION) {
				// process the message
				return $this->wsProcessClientMessage($clientID, $opcode, $data, $this->wsClients[$clientID]['FramePayloadDataLength']);
			}
			else {
				// increase message payload data length
				$this->wsClients[$clientID]['MessageBufferLength'] += $this->wsClients[$clientID]['FramePayloadDataLength'];

				// push frame payload data onto message buffer
				$this->wsClients[$clientID]['MessageBuffer'] .= $data;

				// process the message
				$result = $this->wsProcessClientMessage($clientID, $this->wsClients[$clientID]['MessageOpcode'], $this->wsClients[$clientID]['MessageBuffer'], $this->wsClients[$clientID]['MessageBufferLength']);

				// check if the client wasn't removed, then reset message buffer and message opcode
				if (isset($this->wsClients[$clientID])) {
					$this->wsClients[$clientID]['MessageBuffer'] = '';
					$this->wsClients[$clientID]['MessageOpcode'] = 0;
					$this->wsClients[$clientID]['MessageBufferLength'] = 0;
				}
			...
			if ($opcode != self::WS_OPCODE_CONTINUATION) {
				$this->wsClients[$clientID]['MessageOpcode'] = $opcode;
			}
		}
    ...
    private function wsProcessClientMessage($clientID, $opcode, &$data, $dataLength) {
		// check opcodes
		...
		elseif ($opcode == self::WS_OPCODE_PONG) {
			...
			if ($this->wsClients[$clientID]['PingSentTime'] !== false) {
				$this->wsClients[$clientID]['PingSentTime'] = false;
			}
		}
		elseif ($opcode == self::WS_OPCODE_CLOSE) {
			...
			if ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CLOSING) {
				...
				$this->wsClients[$clientID]['ReadyState'] = self::WS_READY_STATE_CLOSED;
			}
			else {
		...
		elseif ($opcode == self::WS_OPCODE_TEXT || $opcode == self::WS_OPCODE_BINARY) {
			$this->wsOnMessage($clientID, json_decode($data, true), $dataLength, $opcode == self::WS_OPCODE_BINARY);
		}
		else {
			// unknown opcode
			return false;
    ...
    private function wsProcessClientHandshake($clientID, &$buffer) {
	...
	private function wsSendClientMessage($clientID, $opcode, $message) {
		// check if client ready state is already closing or closed
		if ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CLOSING || $this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CLOSED) return true;
    ...
    private function wsSendClientClose($clientID, $status=false) {
		// check if client ready state is already closing or closed
		if ($this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CLOSING || $this->wsClients[$clientID]['ReadyState'] == self::WS_READY_STATE_CLOSED) return true;

		// store close status
		$this->wsClients[$clientID]['CloseStatus'] = $status;
		...
		$this->wsClients[$clientID]['ReadyState'] = self::WS_READY_STATE_CLOSING;
	}

	// client non-internal functions
	protected function wsClose($clientID) {
		return $this->wsSendClientClose($clientID, self::WS_STATUS_NORMAL_CLOSE);
	}

    protected function wsSend($clientID, $message, $binary=false) {
        if ($message && $message != 'null') {
            $this->log("Server send to client $clientID " . $message);
